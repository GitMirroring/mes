...
(define (ast->type o info)
  (define (type-helper o info)
    ...
    (pmatch o
      ...
      ((fixed ,value)
       (let* ((value (string-upcase value))
              (type (cond ((string-suffix? "LLU" value) "unsigned long long")
                          ((string-suffix? "LU" value) "unsigned long")
                          ((string-suffix? "ULL" value) "unsigned long long")
      ...
      ((add ,a ,b)
       (let ((type-a (ast->type a info))
         ...
         (cond ((pointer? type-a) type-a)
               ((pointer? type-b) type-b)
               (else (convert a b info)))))
      ((sub ,a ,b)
       (let ((type-a (ast->type a info))
             (type-b (ast->type b info)))
         (cond ((pointer? type-a) type-a)
               ((pointer? type-b) type-b)
               (else (convert a b info)))))
      ((bitwise-and ,a ,b) (convert a b info))
      ((bitwise-not ,a) (ast->type a info))
      ((bitwise-or ,a ,b) (convert a b info))
      ((bitwise-xor ,a ,b) (convert a b info))
      ((lshift ,a ,b) (ast->type a info))
      ((rshift ,a ,b) (ast->type a info))
      ((div ,a ,b) (convert a b info))
      ((mod ,a ,b) (convert a b info))
      ((mul ,a ,b) (convert a b info))
      ((not ,a) (get-type "int" info))
      ((pos ,a) (ast->type a info))
      ((neg ,a) (ast->type a info))
      ((eq ,a ,b) (get-type "int" info))
      ((ge ,a ,b) (get-type "int" info))
      ((gt ,a ,b) (get-type "int" info))
      ((ne ,a ,b) (get-type "int" info))
      ((le ,a ,b) (get-type "int" info))
      ((lt ,a ,b) (get-type "int" info))

      ;; logical
      ((or ,a ,b) (get-type "int" info))
      ((and ,a ,b) (get-type "int" info))
...
(define (field-offset info struct field)
  (let ((union? (equal? (type:type struct) 'union))
        (fields (type:description struct)))
    (let loop ((offset 0)
               (fields fields))
      (if (null? fields) (error (format #f "no such field: ~a in ~s" field struct))
          (let ((o (car fields))
            ...
            (pmatch o
              ((,name . ,type)
               (guard (equal? field name))
               offset)
              ((bits . ,bit-fields)
               (guard (assoc-ref bit-fields field))
               offset)
              ((,union-or-struct . ,type)
               (guard (and (member union-or-struct '(union struct))
                           (member field (map car (struct->fields type)))))
               (+ offset (field-offset info type field)))
              ((,f . ,type)
               (loop (+ offset (if union? 0
                                   (field:size o info)))
                     rest))))))))
  ...
  (append-text info
               (wrap-as
                (let ((reg-size (->size "*" info)))
                  (let loop ((n n))
                    (cond ((>= n reg-size)
                           (append (as info 'r0-mem->r1-mem)
                                   (as info 'r+value reg-size)
                                   (as info 'r0+value reg-size)
                                   (loop (- n reg-size))))
                          ((> n 4)
                           (append (as info 'long-r0-mem->r1-mem)
                                   (as info 'r+value 4)
                                   (as info 'r0+value 4)
                                   (loop (- n 4))))
                          (else (case n
                                  ...
                                  ((3) (append (as info 'word-r0-mem->r1-mem)
                                               ...
                                               (as info 'r0+value 2)
                                               (loop (- n 2))))
                                  ((2) (append (as info 'word-r0-mem->r1-mem)
                                               (as info 'r+value 2)))
                                  ((1) (append (as info 'byte-r0-mem->r1-mem)
                                               (as info 'r+value 1)))
                                  ((0) '())))))))))
      ...
      (pmatch o
        ...
        ((ge ,a ,b)
         (let* ((type-a (ast->type a info))
                ...
                (size-a (ast->size a info))
                (size-b (ast->size b info))
                (unsigned? (or (and (>= size-a size-b) (unsigned? type-a))
                               (and (>= size-b size-a) (unsigned? type-b))))
                (test->r (if unsigned? 'ae?->r 'ge?->r))
                (info (append-text info (wrap-as (as info test->r))))
                (info (append-text info (wrap-as (as info 'test-r)))))
           info))
...
(define (convert a b info)
  (let ((type-a (promote a info))
        (type-b (promote b info)))
    (cond ((and (pointer? type-a) (pointer? type-b))
           (make-type 'signed (->size "*" info) #f))
          ((pointer? type-a) type-a)
          ((pointer? type-b) type-b)
          (else
           (let ((size-a (type:size type-a))
                 (size-b (type:size type-b)))
             (cond ((equal? type-a type-b) type-a)
                   ((equal? (signed? type-a) (signed? type-b))
                    (if (>= size-a size-b) type-a type-b))
                   ((and (unsigned? type-a) (>= size-a size-b)) type-a)
                   ((and (unsigned? type-b) (>= size-b size-a)) type-b)
                   ((signed? type-a) type-a)
                   (else type-b)))))))
...
(define (cstring->int o)
  (let* ((o (string-upcase o))
         (o (cond ((string-suffix? "LLU" o)  (string-drop-right o 3))
                  ((string-suffix? "LU" o)  (string-drop-right o 2))
                  ((string-suffix? "ULL" o) (string-drop-right o 3))
    ...
    (pmatch o
      ...
      ((comp-decl (decl-spec-list (type-spec ,type)) (comp-declr-list (comp-declr (ptr-declr ,pointer (ary-declr (ident ,name) ,count)))))
       (let ((rank (pointer->rank pointer))
         ...
         (list (cons name (make-c-array (rank+= (ast->type type info) rank) count)))))
  ...
  (pmatch o
    ...
    (((decl-spec-list (stor-spec (typedef)) (type-spec ,type)) (init-declr-list (init-declr (array-of (ident ,name) ,count))))
     (let* ((info (type->info type name info))
            ...
            (type (make-c-array type count)))
       (clone info #:types (acons name type (.types info)))))
    (((decl-spec-list (stor-spec (typedef)) (type-spec ,type)) (init-declr-list (init-declr (ftn-declr (ident ,name) _))))
     (let* ((info (type->info type name info))
            (type (ast->type type info)))
       (clone info #:types (acons name type (.types info)))))
    (((decl-spec-list (stor-spec (typedef)) (type-spec ,type)) (init-declr-list (init-declr (ptr-declr ,pointer (ftn-declr (ident ,name) _)))))
     (let* ((info (type->info type name info))
            (type (ast->type type info))
            (rank (pointer->rank pointer))
            (type (rank+= type rank)))
       (clone info #:types (acons name type (.types info)))))
    (((decl-spec-list (stor-spec (typedef)) (type-spec ,type)) (init-declr-list (init-declr (ftn-declr (ptr-declr ,pointer (ident ,name)) _))))
     (let* ((info (type->info type name info))
            (type (ast->type type info))
            (rank (pointer->rank pointer))
            (type (rank+= type rank)))
...
(define (init-local-field-to-0 size info)
  (let* ((info (allocate-register info))
         (info (append-text info (wrap-as (as info 'swap-r0-r1))))
         (info (value->r1-mem*n info 0 size))
         (info (append-text info (wrap-as (as info 'swap-r0-r1)))))
    (free-register info)))

(define (init-local-field size expr info)
  (let* ((info (expr->register expr info))
         (info (append-text info (wrap-as (as info 'swap-r0-r1))))
         (info (cond
                ((string? expr)
                 (let* ((expr-size (min size (string-length expr)))
                        (info (r0-mem->r1-mem*n info expr-size)))
                   (value->r1-mem*n info 0 (- size expr-size))))
                ((structured-type? (ast->type expr info))
                 (let* ((expr-size (min size (ast->size expr info)))
                        (info (r0-mem->r1-mem*n info expr-size)))
                   (value->r1-mem*n info 0 (- size expr-size))))
                (else
                 (n/r0->r1-mem info (min size (->size "*" info))))))
         (info (append-text info (wrap-as (as info 'swap-r0-r1)))))
    (free-register info)))

(define (string->c-array? array expressions)
  (let* ((type (c-array:type array))
         (char-array? (and (type? type)
                           (not (structured-type? type))
                           (= 1 (type:size type)))))
    (and char-array?
         (pmatch expressions
           (((p-expr (string . ,strings))) (string-join strings ""))
           ((p-expr (string . ,strings)) (string-join strings ""))
           (_ #f)))))

(define (init-local-array array expressions info)
  (let ((init-string (string->c-array? array expressions))
        (n (c-array:count array)))
    (if init-string (init-local-field n init-string info)
        (let* ((type (c-array:type array))
               (size (->size type info))
               (struct? (structured-type? type)))
          (let loop ((n n)
            ...
            (cond
             ((= n 0) info)
             ((null? expressions)
              (init-local-field-to-0 (* n size) info))
             (else
              (let* ((expr (car expressions))
                     (info
                      (if struct? (init-local-fields type '() expr info)
                          (init-local-field size expr info))))
                (loop (1- n) (cdr expressions) info)))))))))

(define (init-local-fields struct remaining-fields inits info)
  (pmatch inits
    ((initzer-list . ,initzers)
     (init-local-fields struct remaining-fields (map cadr initzers) info))
    (,expressions
     (let loop ((fields (if (eq? (type:type struct) 'union)
                            ;; Only initialize the first field of a union.
                            (list (car (type:description struct)))
       ...
       (cond
        ((null? fields)
         (let* ((info
                 (if (eq? (type:type struct) 'union)
                     ;; Set the write-pointer behind the union, in case the
                     ;; first field is smaller than the union size.
                     (let* ((first-field-size
                             (type:size (cdar (type:description struct))))
                            (union-size (type:size struct))
                            (n (- union-size first-field-size)))
                       (if (> n 0)
                           (append-text info (wrap-as (as info 'r+value n)))
                           info))
                     info)))
           (if (null? remaining-fields) info
               (loop remaining-fields '() expressions info))))
        ((null? expressions)
         (let ((info (init-local-field-to-0 (field:size (car fields) info)
                                            info)))
           (loop (cdr fields) remaining-fields '() info)))
        ((and (structured-type? (cdar fields))
              (eq? 'initzer-list (caar expressions)))
         ;; Nested struct and nested initzer-list.
         (let ((info (init-local-fields (cdar fields)
                                        '()
                                        (car expressions)
                                        info)))
           (loop (cdr fields) remaining-fields (cdr expressions) info)))
        ((structured-type? (cdar fields))
         ;; Nested struct but just expressions, a flat initialization.
         (init-local-fields (cdar fields)
                            (cdr fields)
                            expressions
                            info))
        ((and (c-array? (cdar fields))
              (eq? 'initzer-list (caar expressions)))
         ;; Nested array and nested initzer-list.
         (let ((info (init-local-array (cdar fields)
                                       (map cadr (cdar expressions))
                                       info)))
           (loop (cdr fields) remaining-fields (cdr expressions) info)))
        ((c-array? (cdar fields))
         ;; Nested array but just expressions.
         (if (string->c-array? (cdar fields) expressions)
             (let ((info (init-local-array (cdar fields)
                                           (car expressions)
                                           info)))
               (loop (cdr fields) remaining-fields (cdr expressions) info))
             (let* ((n (min (c-array:count (cdar fields))
                            (length expressions)))
                    (array-expressions (take expressions n))
                    (rest-expressions (drop expressions n))
                    (info (init-local-array (cdar fields)
                                            array-expressions
                                            info)))
               (loop (cdr fields) remaining-fields rest-expressions info))))
        (else
         (let ((info (init-local-field (field:size (car fields) info)
                                       (car expressions)
                                       info)))
           (loop (cdr fields) remaining-fields (cdr expressions) info))))))))

(define (init-local-list local expressions info)
  (let* ((type (local:type local))
         (info (allocate-register info))
         (info (append-text
                info
                (wrap-as (as info 'local-ptr->r (local:id local)))))
         (info (cond
                ((null? expressions)
                 (init-local-field-to-0 (->size (local:type local) info)
                                        info))
                ((c-array? type)
                 (init-local-array type expressions info))
                ((and (structured-type? type)
                      (or (eq? 'initzer-list (caar expressions))
                          (not (eq? type
                                    (ast->type (car expressions) info)))))
                 (init-local-fields type '() expressions info))
                ((= 1 (length expressions))
                 (init-local-field (->size (local:type local) info)
                                   (car expressions)
                                   info))
                (else
                 (error "init-local-list not implemented: " type local)))))
    (free-register info)))

(define (init-local local o info)
  (pmatch o
    (() info)
    (#f info)
    (,string (guard (string? string))
             (init-local local `(initzer-list (initzer (p-expr (string ,string)))) info))
    ((initzer-list (initzer ,expr))
     (init-local-list local (list expr) info))
    ((initzer-list . ,initzers)
     (guard (or (structured-type? (local:type local))
                (c-array? (local:type local))))
     (init-local-list local (map cadr initzers) info))
    ((initzer (initzer-list . ,inits))
     (init-local local (cadar o) info))
    (((initzer (initzer-list . ,inits)))
     (init-local local (cadar o) info))
    ((initzer ,init)
     (init-local-list local (list init) info))
    (((initzer ,init))
     (init-local-list local (list init) info))
    (_
     (init-local-list local (list o) info))))
...
(define (local->info type name o init info)
  (let* ((locals (.locals info))
         ...
         (local (if (not array?) local
                    (make-local-entry name
                                      type
                                      (+ (local:id (cdr local))
                                         -1
                                         (quotient (+ size reg-size -1)
                                                   reg-size)))))
         (local (if (not struct?) local
                    (make-local-entry name type
                                      (+ (local:id (cdr local))
                                         (quotient (+ size (1- reg-size))
                                                   reg-size)))))
         (locals (cons local locals))
         (info (clone info #:locals locals))
         (local (cdr local)))
    (init-local local init info)))
  ...
  (pmatch o
    ...
    (((ary-declr (ident ,name)) . ,init)
     (let* ((strings (init->strings init info))
            (info (if (null? strings) info
                      (clone info #:globals (append (.globals info) strings))))
            (expressions (let loop ((o init))
                           (pmatch o
                             (((initzer (initzer-list . ,initzers)))
                              (map loop initzers))
                             ((initzer ,init)
                              (loop init))
                             (((initzer ,init))
                             ...
                             ((initzer-list . ,initzers)
                              (map loop initzers))
                             (_ o))))
            (string (pmatch expressions
                      ((p-expr (string . ,strings))
                       (string-join strings ""))
                      ((p-expr (string ,string))
                       string)
                      (_ #f)))
            (array-from-string? (and (= (->size type info) 1)
                                     (string? string)))
            (count (if array-from-string? (1+ (string-length string))
                       (length expressions)))
            (type (make-c-array type count)))
       (if (.function info) (local->info type name o init info)
           (global->info storage type name o init info))))
...
(define (init->data type o info)
  (pmatch o
    ...
    ((ref-to (i-sel (ident ,field) (cast (type-name (decl-spec-list ,struct) (abs-declr (pointer))) (p-expr (fixed ,base)))))
     (let* ((field-type (ast->type struct info))
            (offset (field-offset info field-type field))
            (base (cstring->int base)))
       (int->bv type (+ base offset) info)))
